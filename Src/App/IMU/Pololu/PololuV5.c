...
IMU_POLOLU_STS ImuPololuReadData( PIMU_POLOLU_DATA pData )
{
	...
	g_bI2cDone = FALSE;
	g_aBuf[0] = 0;
	I2cRead( g_pI2cHandle, MAGNETOMETER_SLAVE_ADDR, LIS_STATUS_REG, g_aBuf, 1U );
	if( FALSE == mpu_WaitReady() )
	{
		return IMU_POLOLU_STS_I2C_NO_RESP;
	}

	if( 0x08 == (g_aBuf[0] & 0x08) )
	{
    	g_bI2cDone = FALSE;
		g_aBuf[0] = 0;
    	I2cRead( g_pI2cHandle, MAGNETOMETER_SLAVE_ADDR, LIS_OUT_X_L, g_aBuf, 6U );
        if( FALSE == mpu_WaitReady() )
		{
			return IMU_POLOLU_STS_I2C_NO_RESP;
		}

		pData->nMagX = g_aBuf[0] | (g_aBuf[1]<<8);
        pData->nMagY = g_aBuf[2] | (g_aBuf[3]<<8);
        pData->nMagZ = g_aBuf[4] | (g_aBuf[5]<<8);
	}
